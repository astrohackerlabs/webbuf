...
pub fn aesgcm_encrypt(plaintext: &[u8], key: &[u8], iv: &[u8]) -> Result<Vec<u8>, String> {
    if iv.len() != 12 {
...
#[cfg_attr(feature = "wasm", wasm_bindgen)]
pub fn aesgcm_decrypt(ciphertext: &[u8], key: &[u8], iv: &[u8]) -> Result<Vec<u8>, String> {
    if iv.len() != 12 {
        return Err("IV must be exactly 12 bytes".to_string());
    }
...
mod tests {
    ...
    fn test_wrong_key_fails() {
        ...
        let ciphertext = aesgcm_encrypt(plaintext, &key1, &iv).unwrap();
        assert!(aesgcm_decrypt(&ciphertext, &key2, &iv).is_err());
    }
    ...
    fn test_tampered_ciphertext_fails() {
        ...
        let mut ciphertext = aesgcm_encrypt(plaintext, &key, &iv).unwrap();
        ciphertext[0] ^= 0x01; // flip one bit
        assert!(aesgcm_decrypt(&ciphertext, &key, &iv).is_err());
    }
    ...
    fn test_wrong_iv_fails() {
        ...
        let ciphertext = aesgcm_encrypt(plaintext, &key, &iv1).unwrap();
        assert!(aesgcm_decrypt(&ciphertext, &key, &iv2).is_err());
    }
    ...
    fn test_empty_plaintext() {
        ...
        let ciphertext = aesgcm_encrypt(&[], &key, &iv).unwrap();
    ...
    fn test_determinism() {
        ...
        let ct1 = aesgcm_encrypt(plaintext, &key, &iv).unwrap();
        let ct2 = aesgcm_encrypt(plaintext, &key, &iv).unwrap();
        assert_eq!(ct1, ct2);
    }
    ...
    fn test_different_keys_different_output() {
        ...
        let ct1 = aesgcm_encrypt(plaintext, &key1, &iv).unwrap();
        let ct2 = aesgcm_encrypt(plaintext, &key2, &iv).unwrap();
        assert_ne!(ct1, ct2);
    }
    ...
    fn test_invalid_iv_length() {
        let key = [0x01u8; 32];
        assert!(aesgcm_encrypt(b"test", &key, &[0u8; 16]).is_err()); // 16 bytes, not 12
        assert!(aesgcm_encrypt(b"test", &key, &[0u8; 8]).is_err());  // 8 bytes, not 12
    }

    #[test]
    fn test_invalid_key_length() {
        let iv = [0x01u8; 12];
        assert!(aesgcm_encrypt(b"test", &[0u8; 24], &iv).is_err()); // 192-bit not supported
        assert!(aesgcm_encrypt(b"test", &[0u8; 10], &iv).is_err()); // invalid
    }
    ...
    fn test_ciphertext_too_short() {
        let key = [0x01u8; 32];
        let iv = [0x02u8; 12];
        assert!(aesgcm_decrypt(&[0u8; 15], &key, &iv).is_err()); // less than 16 bytes
    }
}
